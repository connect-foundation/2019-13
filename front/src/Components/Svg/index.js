...
import BlockModelList from '../../core/blocks/block/blockmodelList';
import CONSTANTS from '../../core/blocks/constants';
import BlockModel from '../../core/blocks/block/blockmodel';
  ...
  return (
    <SVG onWheel={wheelSVG} transform={`translate(${CONSTANTS.BUTTON_AREA_WIDTH},0)`}>
      ...
      {[...blockModelList.getBlockDB().values()].map(block => (
        <ModelBlock block={block} key={block.id} scrollY={scrollY} />
      ))}
      {workspace.topblocks.map(block => <Block block={block} key={block.id} scroll={isMove} />)}
      {!isMove ? null
        : (
          <rect
            width="300"
